 ...
 * 70000+ 佩戴时长、80000+ 消息通知、90000+ 常用工具、100000+ 小齿档案，
 * 业务模块落地时在本枚举中追加。
 */
public enum ErrorCode {
    ...
    /** 天气服务未配置（Host/Key 未注入）。 */
    WEATHER_NOT_CONFIGURED(60007, "天气服务暂未开通"),
    /** 天气服务本月免费额度已达安全水位，暂停调用。 */
    WEATHER_QUOTA_EXCEEDED(60008, "本月天气额度已用完，请稍后再试"),
    /** 天气服务调用失败。 */
    WEATHER_CALL_FAILED(60009, "天气获取失败"),
    ...
    // 100000+ 小齿档案模块
    /** 复诊日程/记录不存在。 */
    CLINIC_VISIT_NOT_FOUND(100001, "复诊记录不存在"),
    /** 花费流水不存在。 */
    CLINIC_EXPENSE_NOT_FOUND(100002, "花费记录不存在"),
    /** 花费分类不存在。 */
    CLINIC_CATEGORY_NOT_FOUND(100003, "花费分类不存在"),
    /** 小齿档案参数不合法（日期/金额/分类等）。 */
    CLINIC_PARAM_INVALID(100004, "小齿档案参数不合法"),
    /** 花费分类名冲突（同用户下已存在同名分类）。 */
    CLINIC_CATEGORY_NAME_CONFLICT(100005, "分类名已存在"),
    /** 系统内置分类不可修改/删除。 */
    CLINIC_BUILTIN_CATEGORY_IMMUTABLE(100006, "内置分类不可修改");
    // ──────────────────────────────────────────────────────────────
